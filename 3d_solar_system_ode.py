...
def r(i, w):
    ...
    ri = w[start_index:end_index].copy()
    return ri
...
def v(i, w):
    start_index = i * number_dimensions
    end_index = (i + 1) * number_dimensions
    vi = w[start_v_index + start_index:end_index].copy()
    return vi
...
w = sp_odeint(f, w0, t, mxstep=50000000, hmin=.0001)
...
colors = ('#ffff00', '#cc0000', '#ff0000', '#00ffff', '#cc0066', '#663300', '#ff3300', '#ff6699', '#9933ff', '#33ccff',)
...
# lim = np.log(lim)
# w_positions = w[:, :start_v_index]
# w_sign = np.sign(w_positions)
# w_positions = np.absolute(w_positions)
# w_positions = np.log(w_positions)
# w_positions = w_positions * w_sign
# w[:, :start_v_index] = w_positions
